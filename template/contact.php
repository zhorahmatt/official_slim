...
if(
	...
	!isset($_POST['contact-website']) ||
    empty($_POST['contact-name']) ||
    empty($_POST['contact-email']) ||
    empty($_POST['contact-website'])
) {
	
	if( empty($_POST['contact-fname']) && empty($_POST['contact-email']) ) {
		...
		if( !isset( $_POST['contact-website'] ) || empty( $_POST['contact-website'] ) ) {
			if( $fields == "" ) {
				$fields .= "Website";
			} else {
				$fields .= ", Website";
	...
	if (!filter_var($_POST['contact-email'], FILTER_VALIDATE_EMAIL) === false) {
		...
		$msg .= "Name: ".$_POST['contact-email']."\r\n";	
		$msg .= "Email: ".$_POST['contact-website']."\r\n";
